...
// Original Author: agl@chromium.org (Adam Langley)
// Original Source: https://github.com/agl/extract-nss-root-certs/blob/master/convert_mozilla_certdata.go
...
func OutputTrustedCerts(out io.Writer, objects []*Object) {
	...
	for _, cert := range certs {
		...
		fmt.Fprint(out, "\n")
		if !trusted {
			fmt.Fprint(out, "# NOT TRUSTED FOR SSL\n")
		}

		fmt.Fprint(out, "# Issuer: "+nameToString(x509.Issuer)+"\n")
		fmt.Fprint(out, "# Subject: "+nameToString(x509.Subject)+"\n")
		fmt.Fprint(out, "# Label: "+label+"\n")
		fmt.Fprint(out, "# Serial: "+x509.SerialNumber.String()+"\n")
		fmt.Fprint(out, "# MD5 Fingerprint: "+fingerprintString(crypto.MD5, x509.Raw)+"\n")
		fmt.Fprint(out, "# SHA1 Fingerprint: "+fingerprintString(crypto.SHA1, x509.Raw)+"\n")
		fmt.Fprint(out, "# SHA256 Fingerprint: "+fingerprintString(crypto.SHA256, x509.Raw)+"\n")
